...
*/
function isPersistentHttpStatus(status) {
	return status === 401 || status === 403;
}
/** Delay until the next allowed live fetch (minimum gap + jitter). */
function nextLiveDelayMs(random = Math.random) {
	const sample = random();
	return MIN_LIVE_INTERVAL_MS + Math.floor((Number.isFinite(sample) ? Math.max(0, Math.min(1, sample)) : 0) * LIVE_JITTER_MS);
}
/**
...
*/
function isLiveDue(nextAllowedAt, now) {
	if (nextAllowedAt === void 0) return true;
...
*/
function applyStaleFallback(lastGood, fresh) {
	if (fresh.status === "ok") {
		const row = {
			...fresh,
			stale: false
	...
	if (fresh.transient === true && lastGood !== void 0 && lastGood.status === "ok") return {
		row: {
			...lastGood,
			stale: true,
...
* a live fetch.
*/
function resolveStatusSnapshot(cached) {
...
var OverlayController = class OverlayController {
	...
	*/
	async refreshProvider(id, label, enabled, force, now, live, throttle) {
		if (!enabled) return {
			id,
			label,
			status: "disabled",
		...
		if (!(force || !throttle || isLiveDue(this.nextLiveAt[id], now) || lastGood === void 0) && lastGood !== void 0) return lastGood;
		const fresh = await live();
		if (throttle) this.nextLiveAt[id] = now + nextLiveDelayMs();
		const decided = applyStaleFallback(lastGood, fresh);
		if (decided.cache !== void 0) this.lastGood[id] = decided.cache;
...
async function route(req, res, ctrl) {
	...
	try {
		if (method === "GET" && path === "/status") return send(res, 200, {
			...ctrl.status(),
			...resolveStatusSnapshot(ctrl.cachedSnapshot())
		});
