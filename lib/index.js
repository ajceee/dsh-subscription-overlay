...
async function fetchCommandcode(apiKey) {
	...
	try {
		...
		const periodStart = subs?.data?.currentPeriodStart ?? void 0;
		const summaryQ = [orgId ? `orgId=${orgId}` : "", periodStart ? `since=${periodStart}` : ""].filter(Boolean).join("&");
		const summaryRaw = await get(`/alpha/usage/summary${summaryQ ? `?${summaryQ}` : ""}`).catch(() => null);
		const items = [];
