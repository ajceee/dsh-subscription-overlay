 ...
 */
export function isPersistentHttpStatus(status) {
    return status === 401 || status === 403;
}
/** Delay until the next allowed live fetch (minimum gap + jitter). */
export function nextLiveDelayMs(random = Math.random) {
    const sample = random();
 ...
 */
export function isLiveDue(nextAllowedAt, now) {
 ...
 */
export function applyStaleFallback(lastGood, fresh) {
    if (fresh.status === 'ok') {
        const row = { ...fresh, stale: false };
 ...
 * a live fetch.
 */
export function resolveStatusSnapshot(cached) {
    if (cached)
        return cached;
